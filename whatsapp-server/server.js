...
async function initializeClient(accountId, userId, supabaseUrl, supabaseKey) {
  ...
  if (clients.has(accountId)) {
    try {
      ...
      if (state !== "CONNECTED") {
        console.log(
          `[Init] Existing client not connected for ${accountId}. Destroying and re-initializing to regenerate QR...`,
        );
        ...
        try {
          ...
          console.warn("[Init] Error destroying existing client:", e?.message || e);
        }
  ...
  try {
    ...
    if (accountData?.proxy_server) {
      ...
      puppeteerConfig.args = puppeteerConfig.args.filter(
        (arg) => arg !== "--no-proxy-server" && arg !== "--proxy-bypass-list=<-loopback>",
      );
      ...
      try {
        ...
        if (srv) {
          try {
            ...
            console.warn("[QR Timeout] Error closing proxy server:", e?.message || e);
          }
  ...
  client.on("ready", async () => {
    ...
    try {
      ...
      await supa.from("whatsapp_accounts").update({ updated_at: new Date().toISOString() }).eq("id", accountId);
        ...
        try {
          ...
          if (media) {
            ...
            if (uploadError) {
              ...
              const { data: publicUrlData } = supa.storage.from("whatsapp-media").getPublicUrl(fileName);
  ...
  client.on("disconnected", async (reason) => {
    ...
    if (srv) {
      try {
        ...
        console.warn("[Disconnect] Error closing proxy server:", e?.message || e);
      }
      proxyServers.delete(accountId);
    }
  ...
  client.on("auth_failure", async (msg) => {
    ...
    if (srv) {
      try {
        ...
        console.warn("[Auth Failure] Error closing proxy server:", e?.message || e);
      }
      proxyServers.delete(accountId);
    }
...
setInterval(
  async () => {
    ...
    for (const [accountId, timestamp] of lastActivity.entries()) {
      ...
      if (idleTime > IDLE_TIMEOUT) {
        console.log(
          `[Auto-cleanup] Client ${accountId} idle for ${Math.floor(idleTime / 60000)} minutes, cleaning up...`,
        );
        const client = clients.get(accountId);
        if (client) {
          try {
